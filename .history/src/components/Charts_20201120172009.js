...
export default function Charts(props) {
    ...
    return (
        <div style={{width:'100%', height:'100vh',position:'relative'}}>
            { 
                data.map((item,i)=>{
                return <Bar key={i} space={i} totalcases={totalcases[date[i]]} total={item.timeline.cases[date[i]]} totalUp={item.timeline.cases[date[count+1]]} totalDown={item.timeline.cases[date[count-1]]}>
                        <Wrap>
                            <Guage key={i} space={i} totalcases={totalcases[date[count]]} total={item.timeline.cases[date[i]]} totalUp={item.timeline.cases[date[count+1]]} totalDown={item.timeline.cases[date[count-1]]} />
